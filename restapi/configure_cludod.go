...
func configureAPI(api *operations.CludodAPI) http.Handler {
	...
	api.APIKeyHeaderAuth = func(token string) (*models.ModelsPrincipal, error) {
		...
		if ok {
			...
			id, ok, err := authz.CheckAuthHeader(token)
			if err != nil {
				api.Logger("[ERROR] Failed to validate API request signature in header against GH: %v", err)
				return nil, errors.New(500, "Failed to validate API request signature in header against GH: %v", err)
			}
			if ok {
				for _, user := range conf.Server.Users {
					if user.ID() == id {
